...
class NetworkTransmissionSimulator:
    ...
    def plot_results(self, figsize=(15, 12)):
        ...
        gs = GridSpec(4, 1, height_ratios=[2, 2, 2, 1])

        # Plot 1: Bandwidth and allocation (w1, w2)
        ax1 = fig.add_subplot(gs[0])
        ax1.plot(times_minutes, self.transmission_history['bandwidth1'], 'b-', label='Bandwidth 1')
        ax1.plot(times_minutes, self.transmission_history['bandwidth2'], 'r-', label='Bandwidth 2')
        ax1.set_ylabel('Bandwidth (Mbps)', color='black')
        ax1.tick_params(axis='y', labelcolor='black')
        ax1.grid(True, alpha=0.3)

        helpSum = self.transmission_history['w1'] + self.transmission_history['w2']
        for i in range(len(helpSum)):
            self.transmission_history['w1'] = 1.0* self.transmission_history['w1'][i] / helpSum[i]
            self.transmission_history['w2'] = self.transmission_history['w2'][i] / helpSum[i]


        # Create second y-axis for packet allocation
        ax1_right = ax1.twinx()
        ax1_right.plot(times_minutes, self.transmission_history['w1'], 'b--', label='w1', alpha=0.7)
        ax1_right.plot(times_minutes, self.transmission_history['w2'], 'r--', label='w2', alpha=0.7)
        ax1_right.set_ylabel('Packet Allocation (w1, w2)', color='gray')
        ...
        for i, time in enumerate(self.probe_history['times']):
            ...
            if self.probe_history['path1'][i]:
                ax1.plot(time_min, 102, 'bv', markersize=8, alpha=0.7)
            if self.probe_history['path2'][i]:
                ax1.plot(time_min, 98, 'r^', markersize=8, alpha=0.7)
        ...
        ax1.set_title('Bandwidth and Packet Allocation Over Time')
        ...
        ax2_right = ax2.twinx()
        ax2_right.plot(times_minutes, self.transmission_history['R'], 'g-', label='Redundancy (R)', alpha=0.7)
        ax2_right.set_ylabel('Redundancy Packets (R)', color='green')
        ...
        ax2.set_title('Packet Loss and Redundancy Over Time')
...
if __name__ == "__main__":
    ...
    simulator = NetworkTransmissionSimulator(
        ...
        simulation_duration=60,  # 60 minutes total simulation
