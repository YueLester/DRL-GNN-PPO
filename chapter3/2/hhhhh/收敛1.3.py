...
def generate_convergence_curve(
        ...
        min_oscillation=0.015
):
    """
    Generate realistic RL convergence curve with continuous fluctuations
    """

    # Base convergence function (sigmoid-like) with delayed convergence
    ...
    if drop_points and drop_magnitudes and recovery_rates:
        for drop_point, magnitude, recovery in zip(drop_points, drop_magnitudes, recovery_rates):
            ...
            if drop_idx < len(curve) - 1:
                # Apply drop with exponential recovery - with reduced magnitude
                recovery_curve = magnitude * np.exp(-(np.arange(len(curve) - (drop_idx + 1))) / (200 * recovery))
                # Limit the maximum drop to 70% of the current value
                max_drop_percentage = 0.7
                max_drop = curve[drop_idx] * max_drop_percentage
                drop_magnitude = min(magnitude, max_drop)
                recovery_curve = drop_magnitude * np.exp(-(np.arange(len(curve) - (drop_idx + 1))) / (200 * recovery))
    ...
    curve = np.minimum(curve, 1)

    return curve


# Generate curves with higher starting values and limited drops
# 1. Non-hierarchical RL (2 links)
non_hier_2 = generate_convergence_curve(
    start_value=0.35,  # Higher starting value
    final_value=0.91,
    ...
    drop_points=[1200],
    drop_magnitudes=[0.15],  # Controlled drop magnitude
    recovery_rates=[0.8],
...
hier_2 = generate_convergence_curve(
    start_value=0.40,  # Higher starting value
    final_value=0.94,
    ...
    drop_points=[1000],
    drop_magnitudes=[0.10],  # Controlled drop magnitude
    recovery_rates=[0.95],
...
non_hier_3 = generate_convergence_curve(
    start_value=0.30,  # Higher starting value
    final_value=0.88,
    convergence_point=2000,
    noise_scale=0.03,
    oscillation_scale=0.05,
    early_oscillation_scale=0.07,
    rising_oscillation_scale=0.09,
    drop_points=[1500],
    drop_magnitudes=[0.18],  # Controlled drop magnitude
    recovery_rates=[0.7],
...
hier_3 = generate_convergence_curve(
    start_value=0.38,  # Higher starting value
    final_value=0.92,
    ...
    rising_oscillation_scale=0.07,
    drop_points=[1800],
    drop_magnitudes=[0.12],  # Controlled drop magnitude
    recovery_rates=[0.9],
...
non_hier_4 = generate_convergence_curve(
    start_value=0.25,  # Higher starting value
    final_value=0.85,
    ...
    drop_magnitudes=[0.20, 0.15],  # Controlled drop magnitudes
    recovery_rates=[0.6, 0.7],
    struggle_phase=(2400, 3200),
    struggle_magnitude=0.07,
...
hier_4 = generate_convergence_curve(
    start_value=0.33,  # Higher starting value
    final_value=0.9,
    ...
    rising_oscillation_scale=0.08,
    drop_points=[1700],
    drop_magnitudes=[0.14],  # Controlled drop magnitude
    recovery_rates=[0.85],
...
# Find the absolute minimum value across all curves to set as the plot "zero"
all_curves = [non_hier_2, hier_2, non_hier_3, hier_3, non_hier_4, hier_4]
absolute_min = min([np.min(curve) for curve in all_curves])
print(f"Absolute minimum value across all curves: {absolute_min:.4f}")

# Add a small buffer to ensure we don't touch zero (5% of the range)
y_min = absolute_min - 0.05 * (1.0 - absolute_min)
y_min = max(y_min, 0.01)  # Ensure y_min is positive but close to zero
...
plt.ylim(y_min, 1)  # Set y minimum to our calculated value
...
for i, link_count in enumerate([2, 3, 4]):
    ...
    ax.set_ylim(y_min, 1)  # Set y minimum to our calculated value
...
non_hier_3_alt = generate_convergence_curve(
    start_value=0.28,  # Higher starting value
    final_value=0.87,
    ...
    rising_oscillation_scale=0.08,
    drop_points=[1450],
    drop_magnitudes=[0.16],  # Controlled drop magnitude
    recovery_rates=[0.65],
...
hier_3_alt = generate_convergence_curve(
    start_value=0.36,  # Higher starting value
    final_value=0.91,
...
non_hier_4_alt = generate_convergence_curve(
    start_value=0.24,  # Higher starting value
    final_value=0.84,
    ...
    drop_magnitudes=[0.18, 0.14],  # Controlled drop magnitudes
    recovery_rates=[0.58, 0.68],
    struggle_phase=(2350, 3150),
    struggle_magnitude=0.07,
    learning_rate_drop=2950,
    min_oscillation=0.025
)

hier_4_alt = generate_convergence_curve(
    start_value=0.32,  # Higher starting value
    final_value=0.89,
    ...
    rising_oscillation_scale=0.08,
    drop_points=[1650],
    drop_magnitudes=[0.13],  # Controlled drop magnitude
    recovery_rates=[0.83],
...
# Find minimum for alternative plots
all_alt_curves = [non_hier_2_alt, hier_2_alt, non_hier_3_alt, hier_3_alt, non_hier_4_alt, hier_4_alt]
alt_min = min([np.min(curve) for curve in all_alt_curves])
print(f"Alternative plots minimum value: {alt_min:.4f}")

# Add buffer to alternative minimum
alt_y_min = alt_min - 0.05 * (1.0 - alt_min)
alt_y_min = max(alt_y_min, 0.01)  # Ensure y_min is positive but close to zero
...
plt.grid(True, alpha=0.3)
plt.xlim(0, MAX_EPISODES)
plt.ylim(alt_y_min, 1)  # Set y minimum to our calculated alternative value
plt.legend(loc='lower right', fontsize=12)
