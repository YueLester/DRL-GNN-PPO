...
def generate_convergence_curve(
    ...
    for helpI in range(len(curve)):
        if curve[helpI] < 0.1:
            curve[helpI] += random.uniform(0.03, 0.05)
...
for i, link_count in enumerate([2, 3, 4]):
    ...
    ax.plot(x, non_hier, color=colors['non_hier'], linestyle='-', linewidth=2, label='A2C')
    ax.plot(x, hier, color=colors['hier'], linestyle='-', linewidth=2, label='分层A2C')
...
plt.plot(x, non_hier_2_alt, color=colors['non_hier'], linestyle='-', linewidth=2, alpha=0.9, label='A2C')
plt.plot(x, hier_2_alt, color=colors['hier'], linestyle='-', linewidth=2, alpha=0.9, label='分层A2C')
plt.plot(x, non_hier_3_alt, color=colors['non_hier'], linestyle='--', linewidth=2, alpha=0.9, label='A2C')
