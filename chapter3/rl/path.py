...
# Example usage showing a 1-hour simulation with 12 time slots (5 minutes each)
if __name__ == "__main__":
    # Create 12 time slots (1 hour total)
    num_slots = 12

    # Create series for average bandwidth, packet loss, and fluctuation level
    # Bandwidth gradually decreases then increases
    avg_bandwidth = [12, 11, 10, 9, 8, 7, 7, 8, 9, 10, 11, 12]

    # Packet loss inversely related to bandwidth
    avg_packet_loss = [0.03, 0.04, 0.05, 0.06, 0.07, 0.08, 0.08, 0.07, 0.06, 0.05, 0.04, 0.03]

    # Fluctuation level varies over time
    fluctuation_level = [1, 2, 3, 4, 5, 4, 3, 2, 1, 2, 3, 4]

    # Create the path
    path = Path(
        avg_bandwidth_series=avg_bandwidth,
        avg_packet_loss_series=avg_packet_loss,
        fluctuation_level_series=fluctuation_level,
    ...
    # Choose a time point (25 minutes into simulation)
    time_point = 25
    idx = int(round(time_point / path.total_duration * (path.total_points - 1)))

    # Get channel state
    state = path.get_state_at_time(time_point)
    print(f"Channel state at {time_point} minutes:")
    print(f"  Bandwidth: {state['bandwidth']:.2f} Mbps")
    print(f"  Packet Loss: {state['packet_loss'] * 100:.2f}%")
    print(f"  Energy Consumption: {state['energy_per_byte'] * 1000:.2f} mJ/byte")

    # Simulate sending one packet
    packet_size = 1500  # bytes
    tx_result = path.transmit_data(packet_size, idx)
    print(f"\nTransmitting single packet of {packet_size} bytes:")
    print(f"  Success: {tx_result['success']}")
    print(f"  Energy consumed: {tx_result['energy_consumed']:.4f} J")
    print(f"  Effective throughput: {tx_result['effective_throughput']:.2f} Mbps")

    # Simulate bulk transmission
    bulk_size = 100000  # bytes
    bulk_result = path.transmit_bulk_data(bulk_size, idx)
    print(f"\nTransmitting bulk data of {bulk_size} bytes:")
    print(f"  Packets sent: {bulk_result['packets_sent']}")
    print(f"  Packets received: {bulk_result['packets_received']}")
    print(
        f"  Bytes received: {bulk_result['bytes_received']} bytes ({bulk_result['bytes_received'] / bulk_size * 100:.2f}%)")
    print(f"  Energy consumed: {bulk_result['energy_consumed']:.4f} J")
    print(f"  Success rate: {bulk_result['success_rate']:.2f}%")
