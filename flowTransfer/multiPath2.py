...
for i in range(simTime):
    cur += random.randint(-2, 9)
    if i > simTime/2:
        cur += random.randint(-9, 2)
        cur += random.randint(-9, 2)
    cur = max(50, cur)
...
for streamId in range(len(sourceMessage)):
    ...
    firstPart1.append(min(1,1.0 * 0.8 * curSize / sourceMessage[streamId]))
...
fig, axs = plt.subplots(4, 1, figsize=(20, 11))
colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd']

axs[0].plot(time, sourceMessage, label="Source")  # o-:圆形
axs[0].set_ylabel('Send Rate(Byte/s)')



axs[1].plot(time, firstPart1, color=colors[0], label="AODV")  # s-:方形
axs[1].plot(time, firstPart2, color=colors[1], label="JamRoute")  # o-:圆形
axs[1].plot(time, firstPart3, color=colors[2], label="AMRBEC")  # o-:圆形
axs[1].plot(time, firstPart4, color=colors[3], label="GWO-DRL1")  # o-:圆形
axs[1].plot(time, firstPart5, color=colors[4], label="GWO-DRL2")  # o-:圆形
axs[1].set_ylabel('Completion Rate')

axs[2].plot(time, secondPart1, color=colors[0], label="AODV")  # s-:方形
axs[2].plot(time, secondPart2, color=colors[1], label="JamRoute")  # o-:圆形
axs[2].plot(time, secondPart3, color=colors[2], label="AMRBEC")  # o-:圆形
axs[2].plot(time, secondPart4, color=colors[3], label="GWO-DRL1")  # o-:圆形
axs[2].plot(time, secondPart5, color=colors[4], label="GWO-DRL2")  # o-:圆形
axs[2].set_ylabel('Redundant Size(Byte)')

axs[3].plot(time, thirdPart1, color=colors[0], label="AODV")  # s-:方形
axs[3].plot(time, thirdPart2, color=colors[1], label="JamRoute")  # o-:圆形
axs[3].plot(time, thirdPart3, color=colors[2], label="AMRBEC")  # o-:圆形
axs[3].plot(time, thirdPart4, color=colors[3], label="GWO-DRL1")  # o-:圆形
axs[3].plot(time, thirdPart5, color=colors[4], label="GWO-DRL2")  # o-:圆形
